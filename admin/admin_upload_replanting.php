...
<form name="import" method="post" enctype="multipart/form-data"><br />
    <h2>Upload Excel File Replanting (Tanam Semula)</h2>
    <div class="ui-state-highlight ui-corner-all" style="margin-top: 20px; padding: 0 .7em;"> <span class="ui-icon ui-icon-info" style="float: left; margin-right: 1em;"></span>
        ...
        <br />  1. Use this template <a href="template/tanam_semula.csv"><strong>here</strong></a>, and fill all data according to column name. Refer <a href="template/tanam_semula-sample.csv"><strong>here</strong></a> if you need sample data.<br />
    ...
    <table width="80%" align="center" >
        ...
        <tr>
            ...
            <td><input type="submit" name="submit" value="Submit" onclick="return confirm('Are you sure to proceed?');" />
                <input type="reset" name="reset" id="reset" value="Reset" onclick="location.href = 'home.php?id=config&sub=admin_upload_replanting'" />
...
//echo $_SESSION['captcha']['code'];
    if ($captcha != $captchasession) {
        echo "<html><script language='javascript'>alert('Invalid CAPTCHA Code! You have entered :" . $captcha . " instead of " . $captchasession . " '); location.href='home.php?id=config&sub=admin_upload_replanting';</script></html>";
    }
    ...
    $this_table = "tanam_semula" . $tahun_short;
    $this_current_table = "tanam_semula" . $tahun_short_semasa;
...
//        $r_add = mysqli_query($qadd, $con);
    ...
    while (($filesop = fgetcsv($handle, 10000, ",")) !== false) {
        ...
        $tanaman_semula = str_replace(",", "", mysqli_real_escape_string($con, $filesop[6])); //$filesop[6];
    ...
    $qa = "delete from $this_table where tanaman_semula='0'; ";
    mysqli_query($con, $qa);
