...
function kos_matang_penjagaan($lesen,$tahun,$jum_tanam,$jum_bts, $negeri,$daerah){
	...
	if($jum_tanam > 0){
		...
		$sub[40]=round($row['total_b_13']/$jum_tanam,2);
		$sub[41]=round($row['total_b_14']/$jum_tanam,2);
	...
	if($jum_bts > 0){
		...
		$sub[42]=round($sub[40]/$jum_bts,2);add_kmp($lesen,$tahun, $negeri,$daerah, $sub[40], $sub[42], "Analisis tanah dan daun");
		$sub[43]=round($sub[41]/$jum_bts,2);add_kmp($lesen,$tahun, $negeri,$daerah, $sub[41], $sub[43], "Penjagaan jalan, parit, ban & pintu air dan sebagainya");
...
function kos_belanja_am ($lesen, $tahun, $hektar, $bts, $negeri, $daerah){
	...
	if($hektar > 0){
		...
		$sub[32]=round($row['pembelian_mesin']/$hektar,2);			//soalan baru perbelanjaan am -pembelian_mesin
		$sub[33]=round($row['pembelian_aset']/$hektar,2);			//soalan baru perbelanjaan am - pembelian_aset
	}
	...
	if($bts > 0){
		...
		$sub[34]=round($sub[32]/$bts,2);add_belanja_am($lesen,$tahun, $negeri,$daerah, $sub[32], $sub[34], "Purchase of machinery and bin for farm use"); 	//soalan baru perbelanjaan am -pembelian_mesin
		$sub[35]=round($sub[33]/$bts,2);add_belanja_am($lesen,$tahun, $negeri,$daerah, $sub[33], $sub[35], "Other expenses not included in expenses 1-7 above");		//soalan baru perbelanjaan am - pembelian_aset
