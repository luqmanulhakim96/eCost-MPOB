...
if(isset($negeri)){
	if(isset($district)){
		...
		header("Content-Disposition: attachment; filename=Estate_Response_Survey_in_$negeri.xls");
	}
}else{
	header("Content-Disposition: attachment; filename=Estate_Response_Survey_in_Peninsular_Malaysia.xls");
}
?>
<style type="text/css">
<!--
...
<table width="100%" border="1" cellpadding="4" cellspacing="0" class="display" id="example" style="border-collapse:collapse">
  ...
  <tbody>
	<?php
		while($row=mysqli_fetch_array($result_peninsular)) { ?>
