...
<div align="center"> <img src="linear_mature_graph.php?table=<?php echo $table ?>&tahun=<?php echo $tahun; ?>&amp;field=<?php echo $field; ?>&type=<?php echo $type; ?>&hold=yes&trim=<?php echo $trim; ?>" height="600" /> </div>

<?php
if ($trim == 'yes') {
    echo "<script>alert('Data has been trimmed');</script>";

    echo "<script>window.location='linear_mature_graph_view.php?table=$table&tahun=$tahun&type=$type&field=$field&type=$type';</script>";
}
?>
...
<?php } ?>
    ...
    <?php
        ...
        $query = "select * from $graf where sessionid='$field' and tahun = '$tahun'  order by status, x asc";
    ...
    $query_avg_draft = "select avg(y) as average  from $graf where sessionid='$field' and tahun = '$tahun'   and (status='0' or status='9')  order by x ";
        ...
        while ($row = mysql_fetch_array($res)) {
            ?>
               <tr <?php
                ...
                ?>>
                <td><?php echo ++$t; ?>.</td>
                <td><input type="checkbox" name="license[]" id="license" value="<?php echo
        $row['x'] . "+" . $row['y'] . "+" . $row['lesen'];
            ...
            <?php echo $row['lesen']; ?></td>
                ...
                <td><div align="right"><?php if ($row['status'] == '0') {
...
if (isset($trim)) {
    echo "<script>window.location=\"linear_mature_graph_view.php?table=$table&tahun=$tahun&type=$type&field=$field&type=$type&trim=yes\";</script>";
}
