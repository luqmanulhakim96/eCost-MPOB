...
<table width="85%" border="0" align="center" class="tajuk">

    <tr>
        <td width="10%" class="tajuk">Select Year :</td>
        <td width="45%" class="tajuk"><form name="form" id="form">
                <select name="jumpMenu" id="jumpMenu" onChange="MM_jumpMenu('parent', this, 0)">
                    <option value="home.php?id=estate&amp;sub=new_planting&amp;year=1&amp;state=<?php echo $state; ?>" <?php if ($year == '1') { ?>selected="selected" <?php } ?>>Year 1</option>
                    <option value="home.php?id=estate&amp;sub=new_planting&amp;year=2&amp;state=<?php echo $state; ?>" <?php if ($year == '2') { ?>selected="selected" <?php } ?>>Year 2</option>
                    <option value="home.php?id=estate&amp;sub=new_planting&amp;year=3&amp;state=<?php echo $state; ?>" <?php if ($year == '3') { ?>selected="selected" <?php } ?>>Year 3</option>
                </select>
            </form>        </td>
        <td width="45%">
...
if ($state != "") {
    ...
    ?>
                <?php if ($totalstate > 0) { ?>
                    ...
                    <h2><?php echo $district; ?></h2><?php }
    } ?>        </td>
    </tr>
    <tr class="tajuk">
        <td colspan="3" valign="top">
<?php if ($rowstate['nama'] != "") { ?>
                <a href="negeri_select.php?id=<?php echo $rowstate['nama']; ?>" class="boxcolor">View By District</a>
<?php } ?>    </td>
    </tr>
    <tr class="tajuk">
      <td colspan="3" valign="top"> <a href="javascript:openScript('mi_new_planting_draft.php?year=<?php echo $year; ?>&state=<?php echo $state; ?>','800','600')" >View Draft</a></td>
    </tr>
</table>

<table width="85%" align="center" class="baju">
    <tr>
        <td colspan="5"><h2><strong>Cost in the <?= $bilTahun[$year - 1] ?> Year of Oil Palm New Planting (RM per hectare)</strong></h2></td>
    </tr>
    <tr >
...
while ($rows = mysqli_fetch_array($rs)) {
    ...
    <tr <?php if (++$gg % 2 == 0) { ?>class="alt"<?php } ?>>
        <td <?php if ($_COOKIE['tahun_report'] == 2010) { ?> ondblclick="javascript:openScript('add_cop.php?name=<?php echo $rows['name']; ?>&type=<?php echo "Penanaman Baru"; ?>&tahun=<?php echo $_COOKIE['tahun_report']; ?>&year=<?php echo $year; ?>&state=<?php echo $state; ?>', '700', '200')"<?php } ?>>
    ...
    if ($rows['isChild'] == 'N' && strlen($rows['sub_type']) > 0) {
        if ($rows['sub_type'] == "weeding") {
            ...
            ?>
                <td width="68"><div align="right"><?php $totalAll1 = $total1[1] + $total2[1] + $total3[1];
            echo number_format($totalAll1, 2); ?></div></td>
                <td width="68"><div align="right"><?php $totalAll2 = $total4[1] + $total5[1] + $total6[1];
            echo number_format($totalAll2, 2); ?></div></td>
                <td width="68"><div align="right"><?php $totalAll3 = $total4[0] + $total5[0] + $total6[0];
            echo number_format($totalAll3, 2); ?></div></td>
            ...
            ?>
                <td width="68"><div align="right"><?php $totalAll1 = $total7[1] + $total8[1] + $total9[1] + $total10[1];
            echo number_format($totalAll1, 2); ?></div></td>
                <td width="68"><div align="right"><?php $totalAll2 = $total11[1] + $total12[1] + $total13[1] + $total14[1];
            echo number_format($totalAll2, 2); ?></div></td>
                <td width="68"><div align="right"><?php $totalAll3 = $total11[0] + $total12[0] + $total13[0] + $total14[0];
            echo number_format($totalAll3, 2); ?></div></td>
                <td width="68"><div align="right"><?php echo number_format((($totalAll2 - $totalAll1) / $totalAll1) * 100, 2); ?></div></td>
                <?php
            }
