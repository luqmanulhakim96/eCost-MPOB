...
<table width="85%" border="0" align="center">
  ...
  <tr>
	...
	?>
	<?php if($totalstate>0){?>
	<img src="../images/<?= $rowstate['negeri_path']; ?>" alt="" name="state" width="91" height="45" class="thinborderfloat" id="state" title="<?= $rowstate['nama'];?>" /><br />
<h2><?php echo $district; ?></h2>
  ...
  <tr class="tajuk">
      <td colspan="3" valign="top"> <a href="javascript:openScript('mi_new_replanting_draft.php?year=<?php echo $year; ?>&state=<?php echo $state; ?>','800','600')" >View Draft</a></td>
    </tr>
</table>

<table width="85%" align="center" class="baju">
<tr>
<td colspan="5"><h2><strong>Cost in the <?=$bilTahun[$year-1]?> Year of Oil Palm New Replanting (RM per hectare)</strong></h2></td>
</tr>
<tr >
  ...
  */
  ...
  <tr <?php if(++$gg%2==0){?>class="alt"<?php } ?>>
	...
	if($rows['isChild'] == 'N' && strlen($rows['sub_type']) > 0){
		if($rows['sub_type'] == "weeding"){
			if($_COOKIE['tahun_report']==2010){
				$total1 = cop ( $rows['name'],  "Penanaman Semula", $year, $state, $district, $_COOKIE['tahun_report']);
			}else{
				$total1 = pertama($dua, "Purchase of weedicides", '0', $state, $district, "Penanaman Semula", $year);
				$total2 = pertama($dua, "Labour cost for weeding", '0', $state, $district, "Penanaman Semula", $year);
				$total3 = pertama($dua, "Machinery use and maintenance", '0', $state, $district, "Penanaman Semula", $year);
			}
    ...
    <?php
			...
			if($_COOKIE['tahun_report']==2010){
				$total7 = cop ( $rows['name'],  "Penanaman Semula", $year, $state, $district, $_COOKIE['tahun_report']);
			}else{
				$total7 = pertama($dua, "Purchase of fertilizer", '0', $state, $district, "Penanaman Semula", $year);
				$total8 = pertama($dua, "Labour cost to apply fertilizers", '0', $state, $district, "Penanaman Semula", $year);
				$total9 = pertama($dua, "Machinery use and maintenances", '0', $state, $district, "Penanaman Semula", $year);
				$total10 = pertama($dua, "Soil and foliar analysis", '0', $state, $district, "Penanaman Semula", $year);
			}
			
			$total11 = pertama($satu, "Purchase of fertilizer", '0', $state, $district, "Penanaman Semula", $year);
			$total12 = pertama($satu, "Labour cost to apply fertilizers", '0', $state, $district, "Penanaman Semula", $year);
			$total13 = pertama($satu, "Machinery use and maintenances", '0', $state, $district, "Penanaman Semula", $year);
			$total14 = pertama($satu, "Soil and foliar analysis", '0', $state, $district, "Penanaman Semula", $year);
	?>
    <td width="68"><div align="right"><?php $totalAll1 = $total7[1]+$total8[1]+$total9[1]+$total10[1]; echo number_format($totalAll1,2);?></div></td>
    <td width="68"><div align="right"><?php $totalAll2 = $total11[1]+$total12[1]+$total13[1]+$total14[1]; echo number_format($totalAll2,2);?></div></td>
    <td width="68"><div align="right"><?php $totalAll3 = $total11[0]+$total12[0]+$total13[0]+$total14[0]; echo number_format($totalAll3,2);?></div></td>
    <td width="68"><div align="right"><?php echo number_format((($totalAll2-$totalAll1)/$totalAll1)*100,2);?></div></td>
    <?php
		}
	}else{
		...
		if($_COOKIE['tahun_report']==2010){
			$a1 = cop ( $rows['name'],  "Penanaman Semula", $year, $state, $district, $_COOKIE['tahun_report']);
		}else{
			$a1 = pertama ($dua, $rows['name'], '0',$state, $district, "Penanaman Semula", $year);
		}
		
		$a= pertama ($satu,  $rows['name'], '0',$state, $district, "Penanaman Semula", $year);
	?>
