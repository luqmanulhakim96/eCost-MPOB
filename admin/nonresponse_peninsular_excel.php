...
extract($_REQUEST);
	...
	if($jenis=="peninsular"){
		...
		if(isset($negeri)){
			$tempat = $negeri;
			header("Content-Disposition: attachment; filename=All_Mill_Non_Response_Survey_in_$negeri.xls");
		}else{
			$tempat = "Peninsular Malaysia";
			header("Content-Disposition: attachment; filename=All_Mill_Non_Response_Survey_in_Peninsular.xls");
		}
	}
	else if($jenis=="sabah"){
		$result_t = $result_sabah_incomplete;
		$tempat = "Sabah";
		header("Content-Disposition: attachment; filename=All_Mill_Non_Response_Survey_in_Sabah.xls");
	}
	else if($jenis=="sarawak"){
		$result_t = $result_sarawak_incomplete;
		$tempat = "Sarawak";
		header("Content-Disposition: attachment; filename=All_Mill_Non_Response_Survey_in_Sarawak.xls");
	}else{
		$tempat = "Malaysia";
		header("Content-Disposition: attachment; filename=All_Mill_Non_Response_Survey_in_Malaysia.xls");
	}
?>
<style type="text/css">
<!--
