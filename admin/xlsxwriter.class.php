...
class XLSXWriter
{
	...
	public function writeSheetRow($sheet_name, array $row, $rowcols_options=null)
	{
		if (empty($sheet_name))
			return;
		...
		if (!empty($rowcols_options)) {
			$row_options = isset($rowcols_options['row']) ? $rowcols_options['row'] : null;
			$cell_options = isset($rowcols_options['col']) ? $rowcols_options['col'] : null;
		} else {
			$row_options = null;
			$cell_options = null;
		}
		...
		$c=0;
		foreach ($row as $v) {
			if (!empty($row_options) && !empty($cell_options) && count($cell_options) > $c) $style = array_merge($row_options, $cell_options[$c]);
			else if (!empty($row_options)) $style = $row_options;
			else if (!empty($cell_options) && count($cell_options) > $c) $style = $cell_options[$c];
			else $style = null;
			if (!empty($cell_options) && count($cell_options) > $c) {
				if (isset($cell_options[$c]['format'])) {
					$number_format = self::numberFormatStandardized($cell_options[$c]['format']);
					$number_format_type = self::determineNumberFormatType($number_format);
				} else {
