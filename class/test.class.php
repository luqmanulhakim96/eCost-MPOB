<?php
	...
	function negeri_mill ($state, $district, $year){
		...
		$total_p_query = "SELECT ekilang.no_lesen lesen, syarikat_induk, nama_kilang, success access, email
							...
							GROUP BY no_lesen";
		$result_total_p = mysqli_query($con, $total_p_query);
		...
		$total_peninsular = mysqli_num_rows($result_all);
		...
		$nonresponse_peninsular = mysqli_num_rows($result_all_incomplete);
	...
	$all_incomplete .= " ORDER BY login_mill.success DESC";
	$result_all_incomplete = mysqli_query($con, $all_incomplete);
	$total_incomplete = mysqli_num_rows($result_all_incomplete);
