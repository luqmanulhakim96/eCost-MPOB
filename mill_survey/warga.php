  ...
  <table width="300px">
    ...
    <tr>
	  ...
	  <?php  $var[3]=$row['kod_warga']; ?>
        <?php if($_SESSION['view']!="true"){ ?>  <input name="warga[<?= $row['kod_warga']; ?>]" type="text" class="field_active" id="warga[<?= $d; ?>]" onchange="kira(this,'<?= $warga->viewwargawhere($var);?>');" value="<?php echo $warga->viewwargawhere($var); ?>" autocomplete="off" onKeypress="keypress(event)" />
<?php } ?>
...
<?php } ?>
    ...
    <tr>
      <td width="26%">&nbsp;</td>
      <td width="74%">
      <?php if($_SESSION['view']!="true"){ ?> <input type="submit" name="Submit" value=<?=setstring ( 'mal', '"   Simpan   " ', 'en', ' "   Save   " '); ?>
/>
